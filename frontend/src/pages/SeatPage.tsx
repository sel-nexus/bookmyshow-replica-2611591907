...
export default function SeatPage() {
  ...
  return (
    <main className="panel">
      <p className="eyebrow">
        {movie.title.toUpperCase()} / {theatre.name.toUpperCase()}
      </p>
      <h1>Find your seats</h1>
      <p aria-live="polite">
        {seats.length
          ? `Selected: ${seats.join(', ')} · Rs. 450`
          : 'No seats selected yet.'}
      </p>
      <div aria-label="Theatre seat grid" className="seat-grid">
        {allSeats.map((seat) => (
          <span
            aria-label={`${seat}${seats.includes(seat) ? ' selected' : ' unselected'}`}
            className={seats.includes(seat) ? 'seat selected' : 'seat'}
            key={seat}
          >
            {seat}
          </span>
      ...
      <button className="button" type="button" onClick={selectSeats}>
        Select Seats
      </button>
    </main>
  );
